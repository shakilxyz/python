...
while True:
    ...
    try:
        ...
        for i in data['list']:
            ...
            temp = str(round(float(i['main']['temp']) - 273, 2)) + u'\N{DEGREE SIGN}'
            min_temp = str(round(float(i['main']['temp_min']) - 273, 2)) + u'\N{DEGREE SIGN}'
            max_temp = str(round(float(i['main']['temp_max']) - 273, 2)) + u'\N{DEGREE SIGN}'
            feel_temp = str(round(float(i['main']['feels_like']) - 273, 2)) + u'\N{DEGREE SIGN}'
            ...
            wind_angle = str(i['wind']['deg']) + u'\N{DEGREE SIGN}'
            ...
            print("""
            Time : {}
                Temperature : {:<6}C         |   Pressure   : {:<4}hpa  |   Wind speed : {} m/s
                Minimum temperature : {:<6}C |   Sea level  : {:<6}   |   Wind angle : {}°
                Maximum temperature : {:<6}C |   Grnd level : {:<6}   |   Sky : {}, clouds {}%
                Feels like : {:<6}C          |   Humidity   : {:<2}%      |   Description : {}""".format(time, temp, pressure, wind_speed, min_temp, sea_level, wind_angle, max_temp, grnd_level, main, clouds, feel_temp, humidity, description))
    except:
